...
public class Practice02HsvEvaluatorLayout extends RelativeLayout{
    ...
    private class HsvEvaluator implements TypeEvaluator<Integer>{
        ...
        public Integer evaluate(float fraction, Integer startValue, Integer endValue){
            ...
            if (outHsv[0] > 360) { outHsv[0]-=360; }else if (outHsv[0] < 0) { outHsv[0]+=360; }
            outHsv[1]=startHsv[1] + (endHsv[1] - startHsv[1]) * fraction;
            outHsv[2]=startHsv[2] + (endHsv[2] - startHsv[2]) * fraction;
            
            
            int alpha=startValue >> 24 + (int) ((endValue >> 24 - startValue >> 24) * fraction); // 把 HSV 转换回 ARGB 返回 return Color.HSVToColor(alpha, outHsv);
